...
using NameHash = Gibbed.Brink.FileFormats.Package.NameHash;

namespace Gibbed.Brink.Setup
{
    public class Project
    {
        ...
        public Dictionary<NameHash, string> FileHashLookup { get; private set; }
        ...
        private Project()
        {
            ...
            this.FileHashLookup = new Dictionary<NameHash, string>();
            this.Loaded = false;
        }
        ...
        public static NameHash HashMD5(string input)
        {
            var md5 = System.Security.Cryptography.MD5.Create();
            var bytes = System.Text.Encoding.ASCII.GetBytes(input);
            return new NameHash(md5.ComputeHash(bytes));
        }

        public void Reload()
        {
            this.Loaded = true;
            this.LoadLists("*.filelist", this.FileHashLookup, s => HashMD5(s.ToLowerInvariant()));
        }

        public string GetFileName(NameHash hash)
        {
        ...
        public string GetFileNameDefault(NameHash hash)
        {
                ...
                using (var input = File.Open(listPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    ...
                    while (true)
                    {
                        ...
                        if (hashes.ContainsKey(hash) == true &&
                            hashes[hash] != line.ToLowerInvariant())
                        {
